...
本项目遵循**规范驱动开发（Spec-Driven Development）**范式。所有新增功能、规则或变更都必须以 `/specs` 目录下的规范文档为依据。

### 贡献规则

1. **先更新规范，再实现变更**
   - 新增规则集或功能前，先查阅 `/specs/product/` 中的产品需求文档
   - 技术实现前，先查看 `/specs/rfc/` 中的技术设计文档
   - API 相关变更，先更新 `/specs/api/` 中的接口定义

2. **AI 助手工作流**
   - 本项目使用 `AGENTS.md` 定义 AI 助手工作流
   - AI 助手必须遵循"先看规范，再写代码"的原则
...
- 📝 在 `/specs/product/` 中提交新功能建议
- 🏗️ 在 `/specs/rfc/` 中编写技术设计方案
- 🔌 在 `/specs/api/` 中更新接口定义
